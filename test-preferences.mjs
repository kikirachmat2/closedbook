...
async function runPreferencesTest() {
  ...
  // Screenshot: Preferences Modal open
  ...
  const idrBtn = await page.evaluateHandle(() => {
    const buttons = Array.from(document.querySelectorAll("button"));
    return buttons.find((b) => b.textContent.includes("IDR"));
  });
  if (idrBtn) {
    await idrBtn.click();
  ...
  const indigoBtn = await page.evaluateHandle(() => {
    const buttons = Array.from(document.querySelectorAll("button"));
    return buttons.find((b) => b.textContent.includes("Cyber Indigo"));
  });
  if (indigoBtn) {
  ...
  const indoLangBtn = await page.evaluateHandle(() => {
    const buttons = Array.from(document.querySelectorAll("button"));
    return buttons.find((b) => b.textContent.includes("Bahasa Indonesia"));
  });
  if (indoLangBtn) {
    await indoLangBtn.click();
    await new Promise((r) => setTimeout(r, 300));
  }
  ...
  const closeBtn = await page.evaluateHandle(() => {
    const buttons = Array.from(document.querySelectorAll("button"));
    return buttons.find((b) => b.textContent.includes("Terapkan Preferensi") || b.textContent.includes("Save Preferences"));
  });
  if (closeBtn) {
